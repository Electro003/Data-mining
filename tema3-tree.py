...
# 5.1 Decision Tree
def optimize_decision_tree(X, y, cv=5):
    print("\n=== Decision Tree Optimization ===")

    param_grid = {
        'criterion': ['gini', 'entropy', 'log_loss'],  # Add log_loss criterion
        'max_depth': [None, 5, 10, 15, 20, 25, 30, 40, 50],  # More depth options
        'min_samples_split': [2, 5, 10, 20, 30],  # More split thresholds
        'min_samples_leaf': [1, 2, 4, 8, 16],  # More leaf size options
        'max_features': ['sqrt', 'log2', None],  # Feature subset options
    ...
    search = RandomizedSearchCV(
        ...
        verbose=1,
        random_state=42,
        return_train_score=True  # To analyze potential overfitting
    )
    search.fit(X, y)
    ...
    best_dt = DecisionTreeClassifier(**search.best_params_, random_state=42)

    best_dt.fit(X, y)

    plt.figure(figsize=(20, 10))
    plot_tree(best_dt, filled=True, max_depth=3)
...
def main():
    ...
    # Perform feature selection before any model training
    selection_method = 'tree'
    n_features = 8
    ...
    # Decision Tree
    best_dt = optimize_decision_tree(X_selected, y)
    best_models['dt'] = best_dt
    dt_results = evaluate_model(best_dt, X_selected, y)
    report_results("Decision Tree", dt_results)

if __name__ == "__main__":
    # Load data
    try:
        # Option 2: Run the main analysis
        main()
