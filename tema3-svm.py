...
def optimize_svm(X, y, cv=5):
    ...
    best_svm = SVC(**grid_search.best_params_, probability=True, random_state=42)
...
def main():
    ...
    best_svm = optimize_svm(X_selected, y)
    best_models['svm'] = best_svm
    svm_results = evaluate_model(best_svm, X_selected, y)
    report_results("Support Vector Machine", svm_results)
