...
def optimize_ensembles(X, y, cv=5):
    ...
    print(f"Best RF cross-validation score: {rf_grid_search.best_score_:.4f}")
    ...
    print(f"Best ET cross-validation score: {et_grid_search.best_score_:.4f}")

    best_et = ExtraTreesClassifier(**et_grid_search.best_params_, random_state=42)
    best_et.fit(X, y)
    ...
    print(f"Best XGB cross-validation score: {xgb_grid_search.best_score_:.4f}")

    best_xgb = xgb.XGBClassifier(**xgb_grid_search.best_params_, objective='binary:logistic', random_state=42)
    best_xgb.fit(X, y)

    # Return best models
    return best_rf, best_et, best_xgb
...
def main():
    ...
    best_rf, best_et, best_xgb = optimize_ensembles(X_selected, y)
    best_models['rf'] = best_rf
    best_models['et'] = best_et
    best_models['xgb'] = best_xgb

    rf_results = evaluate_model(best_rf, X_selected, y)
    et_results = evaluate_model(best_et, X_selected, y)
    xgb_results = evaluate_model(best_xgb, X_selected, y)

    report_results("Random Forest", rf_results)
    report_results("Extra Trees", et_results)
    report_results("XGBoost", xgb_results)
