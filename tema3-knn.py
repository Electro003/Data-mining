...
def main():
    ...
    # KNN
    best_knn = optimize_knn(X_selected, y)
    best_models['knn'] = best_knn
    knn_results = evaluate_model(best_knn, X_selected, y)
    report_results("K-Nearest Neighbors", knn_results)


def compare_feature_selection_methods(df):
    print("\n=== Comparing Feature Selection Methods ===")

    methods = ['tree', 'f_test', 'mutual_info', 'rfe']
    n_features = [2,3,5]  # Fix the number of features for fair comparison
    ...
    for features in n_features:
        ...
        for method in methods:
            ...
            X_selected, y, selected_features, _, _ = preprocess_and_select_features(
                df, selection_method=method, n_features=features
            )

            selected_features_by_method[method] = selected_features

            cv_scores = cross_val_score(model, X_selected, y, cv=5, scoring='f1_weighted')
            avg_score = np.mean(cv_scores)
            std_score = np.std(cv_scores)

            print(f"Average F1 score: {avg_score:.4f} (±{std_score:.4f})")
            results[method] = avg_score

        plt.figure(figsize=(10, 6))
        methods_names = {'tree': 'Tree-based', 'f_test': 'ANOVA F-test',
                         'mutual_info': 'Mutual Information', 'rfe': 'RFE'}

        plt.bar(range(len(results)), [results[m] for m in methods])
        plt.xticks(range(len(results)), [methods_names[m] for m in methods])
        plt.ylabel('F1 Score')
        ...
        print(f"\nBest selection method: {methods_names[best_method]} (F1 = {results[best_method]:.4f})")

    print("\nFeature overlap between different methods:")
    for m1 in methods:
        for m2 in methods:
            if m1 < m2:  # To avoid printing both (A,B) and (B,A)
                set1 = set(selected_features_by_method[m1])
                set2 = set(selected_features_by_method[m2])
                overlap = len(set1.intersection(set2))
                print(f"{methods_names[m1]} and {methods_names[m2]}: {overlap}/{features} features in common")

    return best_method, selected_features_by_method[best_method]
