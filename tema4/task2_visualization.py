...
def visualize_optimal_clustering_results(features, method_results, dataset_name, output_dir, linkage_method='ward'):
    for method, optimal_k in method_results.items():
        if 'KMeans' in method:
            algorithm = 'KMeans'
            clustering = KMeans(n_clusters=optimal_k, random_state=42, n_init=10)
...
def plot_silhouette_visualization(features, optimal_k, algorithm, dataset_name, output_dir, linkage_method='ward'):
    ...
    if algorithm == 'hierarchical' and linkage_method != 'ward':
    ...
    # Adjust filename based on linkage method
    filename_suffix = ''
    if algorithm == 'hierarchical' and linkage_method != 'ward':
